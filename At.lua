...
local Settings = {
    Aimbot = {
        ...
        HitboxSize = 25,
        WallBang = false,
        Smoothness = 0.1, -- 0.05 = سريع, 0.1 = متوسط, 0.2 = بطيء
        AimSpeed = "Medium" -- "Fast", "Medium", "Slow"
    },
    ESP = {
        Enabled = false,
        ShowNames = false,
...
-- ==========================================
-- 2. بناء الواجهة الرئيسية (Main GUI)
-- ==========================================
...
MainFrame.Size = UDim2.new(0, 256, 0, 340)
...
for i = 1, 25 do
    ...
    local randomY = math.random() * 1.1 - 0.1
...
RunService.RenderStepped:Connect(function(dt)
    local deltaTime = math.min(dt, 0.1)
    for char, data in pairs(matrixChars) do
        if char and char.Parent then
            ...
            if newY > 1.05 then
                ...
                char.TextTransparency = math.random(3, 7) / 10
                char.TextColor3 = Color3.fromRGB(0, math.random(120, 255), math.random(80, 200))
                data.Speed = math.random(15, 45) / 100
            end
...
StatusText.Size = UDim2.new(0, 75, 0, 12)
StatusText.Position = UDim2.new(0, 155, 0.5, -6)
StatusText.BackgroundTransparency = 1
StatusText.Text = "● Online • 60 FPS"
...
RunService.RenderStepped:Connect(function(dt)
    ...
    if fpsTimer >= 1 then
        ...
        StatusText.Text = "● Online • " .. FPS .. " FPS"
        frameCount = 0
        fpsTimer = 0
    end
end)
...
MinBtn.MouseButton1Click:Connect(function()
    ...
    TweenService:Create(MainFrame, TweenInfo.new(0.4, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), 
        {Size = isMinimized and UDim2.new(0, 256, 0, 36) or UDim2.new(0, 256, 0, 340)}):Play()
...
local ProtectionScroll = CreateScrollContainer("ProtectionScroll")
...
local function CreateTabButton(text, icon, targetScroll)
    ...
    btn.Size = UDim2.new(0, btn.TextBounds.X + 18, 1, -4)
    ...
    btn.MouseButton1Click:Connect(function()
        local tabs = {
            ...
            TeleportScroll, TransmissionScroll, FarmScroll, AutoClickerScroll, ProtectionScroll
        }
...
CreateTabButton("Protect", "🛡️", ProtectionScroll)
...
-- زر اختيار مستوى السرعة للأيمبوت
local function CreateSpeedSelector(parentScroll)
    local container = Instance.new("Frame")
    container.Size = UDim2.new(1, -8, 0, 28)
    container.BackgroundColor3 = Color3.fromRGB(45, 45, 50)
    container.BackgroundTransparency = 0.1
    ...
    speedLabel.Position = UDim2.new(0.55, 0, 0, 0)
    speedLabel.BackgroundTransparency = 1
    speedLabel.Text = "Medium"
    speedLabel.TextColor3 = Color3.fromRGB(50, 255, 50)
    ...
    local speedValues = {Slow = 0.2, Medium = 0.1, Fast = 0.05}
    local currentIndex = 2
    
    local function updateSpeed()
        local speed = speeds[currentIndex]
        speedLabel.Text = speed
        Settings.Aimbot.AimSpeed = speed
        Settings.Aimbot.Smoothness = speedValues[speed]
        if speed == "Fast" then
...
CreateSpeedSelector(AimbotScroll)
...
CreateToggleItem(ComboScroll, "Jump Power Boost", function(s) Settings.Combo.JumpPowerBoost = s end)
CreateInputItem(ComboScroll, "Jump Multiplier:", "2", function(val) Settings.Combo.JumpPowerMultiplier = val end)
...
-- Protection Tab
CreateToggleItem(ProtectionScroll, "Anti-Kick", function(s) Settings.Protection.AntiKick = s end)
CreateToggleItem(ProtectionScroll, "Anti-Ban", function(s) Settings.Protection.AntiBan = s end)
CreateToggleItem(ProtectionScroll, "FPS Boost", function(s)
    Settings.Protection.FPSBoost = s
    if s then
        -- تقليل جودة الرسوميات لتحسين الأداء
        settings().Rendering.QualityLevel = 1
        workspace.Terrain.WaterWaveSize = 0
        workspace.Terrain.WaterWaveSpeed = 0
        workspace.Terrain.WaterReflectance = 0
        workspace.Terrain.WaterTransparency = 0
        LightTechnology = Enum.Technology.Compatibility
        ShowNotification("🚀 FPS Boost Activated", 2)
    else
        settings().Rendering.QualityLevel = 7
        ShowNotification("📊 Default Graphics Restored", 2)
    end
end)
...
-- ==========================================
-- 14. نظام الحماية (Anti-Kick / Anti-Ban)
-- ==========================================
local oldNamecall
oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
    ...
    if Settings.Protection.AntiKick or Settings.Protection.AntiBan then
        -- منع محاولات الطرد
        ...
        -- منع محاولات الحظر
        if string.find(string.lower(method), "ban") and not checkcaller() then
            return nil
        end
        
        -- حماية من بعض الـ Remotes الضارة
        if self == LocalPlayer and (method == "Destroy" or method == "Remove") and not checkcaller() then
            return nil
        end
    end
...
-- حماية إضافية للاتصال
if Settings.Protection.AntiKick then
...
-- ==========================================
-- 15. المنطق البرمجي (Aimbot, ESP, Combo)
-- ==========================================
-- Aimbot محسن مع Smoothing و Wall Check
            ...
            if head then
                ...
                if onScreen then
                    ...
                    if dist <= shortestDist then
                        -- فحص الجدران (Wall Check)
                        if not Settings.Aimbot.WallBang then
...
RunService:BindToRenderStep("HasanAimbotCore", Enum.RenderPriority.Camera.Value + 1, function()
    ...
    if Settings.Aimbot.Enabled and targetPart then
        -- Smooth Aim باستخدام Lerp
        local targetCFrame = CFrame.lookAt(Camera.CFrame.Position, targetPart.Position)
        local smoothness = Settings.Aimbot.Smoothness
        
        if lastAimCFrame then
            Camera.CFrame = lastAimCFrame:Lerp(targetCFrame, 1 - smoothness)
        else
            Camera.CFrame = targetCFrame
        end
        lastAimCFrame = Camera.CFrame
    else
        lastAimCFrame = nil
    end
...
-- ==========================================
-- 16. Combo Logic محسن (God Mode مطور + Jump Power مصلح)
-- ==========================================
RunService.Stepped:Connect(function()
    ...
    if hum then
        ...
        -- God Mode مطور: تثبيت الصحة ومنع استقبال الضرر
        if Settings.Combo.GodMode then
            if hum.Health > 0 then
                hum.Health = hum.MaxHealth
            end
            -- منع تغيير حالة الـ Humanoid
...
-- God Mode إضافي: منع الضرر عبر Heartbeat
RunService.Heartbeat:Connect(function()
    if Settings.Combo.GodMode and LocalPlayer.Character then
        local hum = LocalPlayer.Character:FindFirstChildOfClass("Humanoid")
        if hum and hum.Health > 0 then
            -- حماية إضافية من خلال تعديل TakeDamage
...
-- ==========================================
-- 17. عرض رسالة الترحيب
-- ==========================================
task.wait(3)
ShowNotification("🚀 Hasan Hub v2.1 Loaded Successfully", 3)
